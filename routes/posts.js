...
function normalizePostVideoUrl(value) {
  ...
  return /^\/uploads\/videos\/video_[A-Za-z0-9_-]+\.(?:mp4|webm|ogv)$/i.test(pathname) ? pathname : null;
}

function normalizePostVideoPosterUrl(value) {
  if (value === undefined || value === null || value === '') return null;
  const pathname = String(value).split('?')[0];
  return /^\/uploads\/posts\/post_[A-Za-z0-9_-]+\.(?:jpg|jpeg|png|gif|webp)$/i.test(pathname) ? pathname : null;
}
...
router.get('/search-users', auth, async (req, res) => {
  try {
    const { q } = req.query;
    const { limit: userLimit } = getPagination(req.query, { defaultLimit: 20, maxLimit: 50 });
  ...
  try {
    ...
    try {
      const [posts] = await pool.execute(
        'SELECT COUNT(*) as total FROM posts WHERE user_id = ? AND status = "approved" AND is_deleted = 0',
        [userId]
      );
    ...
    try {
      const [likes] = await pool.execute(`
        ...
        WHERE user_id = ? AND status = "approved" AND is_deleted = 0
      `, [userId]);
...
router.get('/', optionalAuth, async (req, res) => {
  try {
    const { offset, category, sort = 'latest', keyword, user_id } = req.query;
    const pagination = getPagination(req.query, { defaultLimit: 10, maxLimit: 50 });
    // 无限滚动使用 offset，普通分页使用 page；两者都要限制在合理范围内。
    const requestedOffset = Number.parseInt(offset, 10);
    const finalOffset = Number.isFinite(requestedOffset) && requestedOffset >= 0
      ? Math.min(requestedOffset, 100000)
      : pagination.offset;
    const finalLimit = pagination.limit;
    ...
    // 列表与总数并行，减少总数查询对首屏响应的额外等待。
    const [[posts], [countResult]] = await Promise.all([
      pool.execute(`
        SELECT p.id, p.title, p.content, p.images, p.video_url, p.video_poster, p.is_anonymous, p.category,
      ...
      pool.execute(`SELECT COUNT(*) as total FROM posts p WHERE ${whereClause}`, params)
    ]);
    ...
    const processedPosts = posts.map(post => ({
      ...
      video_url: normalizePostVideoUrl(post.video_url),
      video_poster: normalizePostVideoPosterUrl(post.video_poster),
    ...
    res.json({
      ...
      data: {
        ...
        totalPages: Math.ceil(countResult[0].total / finalLimit)
      }
    });
  } catch (err) {
...
router.get('/user/my', auth, async (req, res) => {
  try {
    const { page, limit, offset } = getPagination(req.query, { defaultLimit: 10, maxLimit: 50 });
...
router.get('/user/favorites', auth, async (req, res) => {
  try {
    ...
    const [countResult] = await pool.execute('SELECT COUNT(*) as total FROM favorites f JOIN posts p ON f.post_id = p.id WHERE f.user_id = ? AND p.is_deleted = 0', [req.user.id]);
...
router.post('/', auth, async (req, res) => {
  try {
    const { title, content, images, video_url, video_poster, is_anonymous, category, contact, pollOptions, pollType, pollExpiresAt, client_ip } = req.body;
...
router.post('/:id/comments', auth, async (req, res) => {
  try {
    ...
    setImmediate(async () => {
      try {
        const [posts] = await pool.execute('SELECT p.user_id, p.title, u.email, u.nickname, u.username FROM posts p LEFT JOIN users u ON p.user_id = u.id WHERE p.id = ? AND p.status = "approved" AND p.is_deleted = 0', [req.params.id]);
  ...
  try {
    ...
    const [replies] = await pool.execute(
      `SELECT r.*, u.nickname, u.username, u.avatar, u.role as author_role
       ...
       WHERE r.comment_id = ? AND c.post_id = ? AND p.status = "approved" AND p.is_deleted = 0
       ORDER BY ${orderBy}`,
...
router.delete('/:postId/comments/:commentId/replies/:replyId', auth, async (req, res) => {
  try {
    ...
    const [replies] = await pool.execute(
      ...
      [replyId, commentId, req.params.postId]
    );
    if (replies.length === 0) {
...
router.delete('/:id', auth, async (req, res) => {
  try {
    const [posts] = await pool.execute('SELECT user_id, is_deleted FROM posts WHERE id = ?', [req.params.id]);
    if (posts.length === 0) {
      return res.json({ code: 404, message: '帖子不存在' });
    }
    ...
    if (posts[0].is_deleted) {
      return res.json({ code: 404, message: '帖子已在回收站' });
    }
    // 用户端删除也进入回收站，由统一清理任务在保留期后彻底删除。
    const [result] = await pool.execute(
      'UPDATE posts SET is_deleted = 1, deleted_at = NOW() WHERE id = ? AND is_deleted = 0',
      [req.params.id]
    );
    res.json({
      code: result.affectedRows > 0 ? 200 : 404,
      message: result.affectedRows > 0 ? '已移入回收站' : '帖子已被其他操作删除'
    });
  } catch (err) {
    res.json({ code: 500, message: '服务器错误' });
  }
...
router.get('/:id/edit', auth, async (req, res) => {
  try {
    const [posts] = await pool.execute(
      'SELECT id, title, content, category, images, video_url, video_poster, is_anonymous, user_id, poll_type, poll_expires_at FROM posts WHERE id = ? AND is_deleted = 0',
      [req.params.id]
    );
...
router.get('/:id/poll', optionalAuth, async (req, res) => {
  try {
    var postId = req.params.id;
    var [posts] = await pool.execute('SELECT id FROM posts WHERE id = ? AND status = "approved" AND is_deleted = 0', [postId]);
