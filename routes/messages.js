    ...
    const html = `
      <div style="max-width:560px;margin:0 auto;font-family:-apple-system,'Segoe UI',Roboto,sans-serif;">
        ...
        <div style="background:#fff;padding:28px 24px;border-radius:0 0 16px 16px;box-shadow:0 4px 20px rgba(255,107,157,0.1);">
          <p style="color:#666;font-size:15px;margin:0 0 12px;"><strong style="color:#FF6B9D;">${safeSenderName}</strong> 给您发了一条私信：</p>
          ...
          <a href="https://campus-wall.example/messages.html" style="display:inline-block;background:linear-gradient(135deg,#FF6B9D,#C084FC);color:#fff;padding:12px 28px;border-radius:24px;text-decoration:none;font-weight:600;font-size:14px;">查看私信 →</a>
        </div>
      </div>
    `;
    ...
    const [conversations] = await pool.execute(`
      SELECT 
        ...
        (SELECT COUNT(*) FROM messages
         WHERE conversation_id = c.id AND sender_id != ? AND is_read = 0 AND deleted_at IS NULL
           AND created_at > COALESCE(CASE WHEN c.user1_id = ? THEN c.user1_cleared_at WHEN c.user2_id = ? THEN c.user2_cleared_at END, '1970-01-01')) as unread_count
...
router.get('/conversations/user/:userId', auth, async (req, res) => {
  try {
    ...
    if (!Number.isInteger(targetUserId) || targetUserId <= 0) {
      return res.status(400).json({ code: 400, message: '用户参数无效' });
    }
    ...
    // 依赖唯一键原子获取/创建，避免两个并发请求同时查空后插入重复会话。
    const [result] = await pool.execute(
      'INSERT INTO conversations (user1_id, user2_id, last_message_at, created_at) VALUES (?, ?, NOW(), NOW()) ' +
      'ON DUPLICATE KEY UPDATE id = LAST_INSERT_ID(id), last_message_at = NOW()',
...
router.get('/conversations/:conversationId/messages', auth, async (req, res) => {
  try {
    ...
    // 标记对方发送的未读消息为已读
    const readFilter = clearedAt ? ' AND created_at > ?' : '';
    const readParams = [conversationId, userId];
    if (clearedAt) readParams.push(clearedAt);
    await pool.execute(
      'UPDATE messages SET is_read = 1, read_at = NOW() ' +
      'WHERE conversation_id = ? AND sender_id != ? AND is_read = 0 AND deleted_at IS NULL' + readFilter,
      readParams
    );
