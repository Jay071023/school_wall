...
function initMessagesPage() {
  ...
  startMessagePolling();
  initSettings();
  syncMobileConversationLayout();
}

// 视口变化时同步会话列表/聊天区，避免从手机旋转到桌面后残留 hide/show 状态。
function syncMobileConversationLayout() {
...
function setupEventListeners() {
  ...
  window.addEventListener('resize', syncMobileConversationLayout, { passive: true });
  document.addEventListener('visibilitychange', function() {
    if (document.hidden) {
      stopMessagePolling();
    } else {
      startMessagePolling();
      if (currentConversationId) checkNewMessages();
    }
  });
}
...
async function loadConversations() {
  try {
    ...
    if (data.code === 200) {
      ...
      dom.conversationList.removeAttribute('aria-busy');
    ...
    showError(dom.conversationList, '加载会话失败，请重试', loadConversations);
  }
}
...
async function loadMessages(conversationId, forceRefresh = false) {
  ...
  try {
    ...
    if (data.code === 200) {
      ...
      dom.messagesList.removeAttribute('aria-busy');
    ...
    showError(dom.messagesList, '加载消息失败，请重试', function() {
      currentPage = 1;
      totalPages = 1;
      loadMessages(currentConversationId, true);
    });
...
async function loadRecentUsers() {
  try {
    ...
    showError(dom.userList, '加载用户失败，请重试', loadRecentUsers);
  }
}
...
async function searchUsers() {
  ...
  try {
    ...
    showError(dom.userList, '搜索失败，请重试', searchUsers);
  }
}
...
function startMessagePolling() {
  if (messageInterval || document.hidden) return;
  // 每30秒检查一次新消息
...
async function updateConversationsUnread() {
  try {
    ...
    if (data.code === 200) {
      ...
      // 当前没有打开会话或页面在后台时，刷新列表显示；打开会话时避免重绘打断阅读。
...
function showError(container, message, retry) {
  ...
  container.innerHTML = `
    <div class="conversation-empty state-error" role="alert">
      <div class="conversation-empty-icon">😢</div>
      <h3 class="conversation-empty-text">${escapeHtml(message)}</h3>
      <button class="state-retry" type="button">重新加载</button>
    </div>
  `;
  var button = container.querySelector('.state-retry');
  if (button && typeof retry === 'function') button.addEventListener('click', retry);
}
