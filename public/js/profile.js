...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function renderPointsUnavailable() {
    var infoEl = document.getElementById('profileInfo');
    if (infoEl) infoEl.innerHTML = '<div class="profile-points-info" style="color:var(--text-light);font-size:0.82rem;text-align:center;">积分信息暂时无法加载</div>';
  }
  ...
  window.retryProfileTab = function() {
    loadTabData(false);
  };
  ...
  async function loadMyPosts(append) {
    ...
    if (!append && postListEl) {
      postListEl.innerHTML = '<div class="profile-tab-state profile-tab-loading" role="status" aria-live="polite"><span class="loading-spinner" aria-hidden="true"></span><p>加载中...</p></div>';
    }
    ...
    try {
      ...
      if (data.code === 200) {
        ...
        if (!append && postListEl) postListEl.innerHTML = '<div class="profile-tab-state profile-tab-error" role="alert"><p>' + escapeHtml(data.message || '内容暂时无法加载') + '</p><button type="button" onclick="window.retryProfileTab()">重新加载</button></div>';
        showToast(data.message || '加载失败', 'error');
      }
    } catch (err) {
      isLoading = false;
      console.error('加载我的帖子失败:', err);
      if (!append && postListEl) postListEl.innerHTML = '<div class="profile-tab-state profile-tab-error" role="alert"><p>网络不稳定，请稍后重试</p><button type="button" onclick="window.retryProfileTab()">重新加载</button></div>';
  ...
  async function loadMyFavorites(append) {
    ...
    try {
      ...
      if (data.code === 200) {
        ...
        if (!append && postListEl) postListEl.innerHTML = '<div class="profile-tab-state profile-tab-error" role="alert"><p>' + escapeHtml(data.message || '收藏暂时无法加载') + '</p><button type="button" onclick="window.retryProfileTab()">重新加载</button></div>';
        showToast(data.message || '加载失败', 'error');
      }
    } catch (err) {
      isLoading = false;
  ...
  async function loadMySongs(append) {
    ...
    try {
      ...
      if (data.code === 200) {
        ...
        if (!append && postListEl) postListEl.innerHTML = '<div class="profile-tab-state profile-tab-error" role="alert"><p>' + escapeHtml(data.message || '点歌记录暂时无法加载') + '</p><button type="button" onclick="window.retryProfileTab()">重新加载</button></div>';
        showToast(data.message || '加载失败', 'error');
      }
    } catch (err) {
      isLoading = false;
  ...
  function renderPostCard(post) {
    ...
    // 个人中心与首页/详情保持同一套安全媒体白名单，只展示本站受控的视频路径。
    var videoUrl = typeof post.video_url === 'string' && /^\/uploads\/videos\/video_[A-Za-z0-9_-]+\.(?:mp4|webm|ogv)$/i.test(post.video_url)
      ? post.video_url
      : '';
    var videoHtml = videoUrl
      ? '<div class="post-video-wrap profile-post-video-wrap" onclick="event.stopPropagation()"><video class="post-video" controls preload="metadata" playsinline src="' + escapeHtml(videoUrl) + '"></video></div>'
      : '';
    ...
    return '<article class="post-card profile-my-post" data-id="' + post.id + '" onclick="window.location.href=\'/post/' + post.id + '\'">' +
      '<div class="profile-my-post-head">' +
        ...
        '</div>' +
      '</div>' +
      '<div class="post-content">' + convertContentWithLinks(post.content || '') + '</div>' +
      imageHtml +
      videoHtml +
      pollPreviewHtml +
...
async function loadNotifications(append) {
  ...
  if (!append && notificationsContainer) {
    notificationsContainer.innerHTML = '<div class="profile-tab-state profile-tab-loading" role="status" aria-live="polite"><span class="loading-spinner" aria-hidden="true"></span><p>加载中...</p></div>';
  }
  ...
  try {
    ...
    if (data.code === 200) {
      ...
      if (!append && notificationsContainer) notificationsContainer.innerHTML = '<div class="profile-tab-state profile-tab-error" role="alert"><p>' + escapeHtml(data.message || '通知暂时无法加载') + '</p><button type="button" onclick="window.retryProfileNotifications()">重新加载</button></div>';
      showToast(data.message || '加载失败', 'error');
    }
  } catch (err) {
    notificationsLoading = false;
    console.error('加载通知失败:', err);
    if (!append && notificationsContainer) notificationsContainer.innerHTML = '<div class="profile-tab-state profile-tab-error" role="alert"><p>网络不稳定，请稍后重试</p><button type="button" onclick="window.retryProfileNotifications()">重新加载</button></div>';
    showToast('网络错误', 'error');
  }
}

window.retryProfileNotifications = function() {
...
function renderNotifications(notifications, append) {
  ...
  notifications.forEach(function(notification) {
    ...
    var readAction = notification.is_read === 0
      ? '<button class="btn-read" onclick="markAsRead(' + notification.id + ', this)">已读</button>'
      : '<span class="notification-read-state">已读</span>';
...
async function markAsRead(notificationId, btn) {
  try {
    var data = await authFetch('/api/notifications/' + notificationId + '/read', { method: 'PUT' });
    if (data.code === 200) {
  ...
  try {
    ...
    if (data.code === 200) {
      ...
      items.forEach(function(item) {
        item.classList.remove('unread');
        var readBtn = item.querySelector('.btn-read');
        if (readBtn) {
          readBtn.textContent = '已读';
          readBtn.disabled = true;
        }
      });
      if (typeof window.updateNotificationBadge === 'function') window.updateNotificationBadge(0);
    } else {
    ...
    try {
      ...
      if (tabContent) {
        ...
        fetch('/api/posts?user_id=' + userId + '&limit=20&t=' + Date.now())
          ...
          .catch(function() { tabContent.innerHTML = '<div style="text-align:center;padding:40px;color:#999;">加载失败</div>'; });
      }
    } catch (err) {
      // 渲染出错时，把错误信息显示在页面上
...
window.loadFeedbackHistory = function() {
  ...
  fetch('/api/feedback/my', { headers: { 'Authorization': 'Bearer ' + (localStorage.getItem('token') || '') } })
    ...
    .then(function(d) {
      ...
      container.innerHTML = '<div style="text-align:center;padding:8px;color:var(--text-light);font-size:0.8rem;">反馈记录暂时无法加载</div>';
    });
};
