...
assert(admin.includes("require('../services/song-scheduling')") && scheduling.includes('createCustomSlotDate'), '无预约日期的旧点歌必须通过排期服务创建单日播放日期');
assert(admin.includes('requestedPlayDate') && scheduling.includes('requestedPlayDate') && adminHtml.includes('play_date: customDate'), '审核和改期必须支持管理员指定播放日期');
...
assert(admin.includes('allowOverbook') && scheduling.includes('if (!allowOverbook && Number(countRows[0].cnt)'), '管理员超额排入必须由后端显式开关控制');
