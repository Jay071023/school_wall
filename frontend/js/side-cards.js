 ...
 */
(function() {
  var SIDE_CACHE_PREFIX = 'campus-wall-side-cache:';

  function readSideCache(key, maxAge) {
    try {
      var cached = JSON.parse(localStorage.getItem(SIDE_CACHE_PREFIX + key) || 'null');
      if (cached && cached.time && Date.now() - cached.time < maxAge) return cached.value;
    } catch (err) {
      // 隐私模式或存储空间不足时直接回退网络请求。
    }
    return null;
  }

  function writeSideCache(key, value) {
    try {
      localStorage.setItem(SIDE_CACHE_PREFIX + key, JSON.stringify({ time: Date.now(), value: value }));
    } catch (err) {
      // 缓存失败不应影响侧栏正常加载。
    }
  }
  ...
  function initAll() {
        ...
        fetch('https://v1.hitokoto.cn/?c=a&c=b&c=d&c=i&c=k')
          ...
          .then(function(data) {
            if (data.hitokoto) {
              ...
              if (sideEl) sideEl.textContent = text.length > 20 ? text.substring(0, 20) + '…' : text;
    ...
    (function() {
      ...
      function load(force) {
        if (!force) {
          var cached = readSideCache('weather', 3 * 60 * 1000);
          if (cached) {
            render(cached);
            return;
          }
      ...
      function render(data) {
        var city = data && data.city ? getCityName(data.city) : '上海';
      ...
      function showDefault() {
        ...
        if (weatherCondition) weatherCondition.textContent = '暂不可用';
      ...
      setTimeout(load, 100);
      if (sideWeather) sideWeather.addEventListener('click', function() { if (weatherCondition) weatherCondition.textContent = '加载中...'; load(true); });
    })();
    ...
    (function() {
      ...
      function load(force) {
        if (!force) {
          var cached = readSideCache('history', 5 * 60 * 1000);
          if (cached) {
            render(cached);
            return;
          }
      ...
      setTimeout(load, 100);
      if (refreshBtn) refreshBtn.addEventListener('click', function(e) { e.stopPropagation(); load(true); });
      if (sideHotsearch) sideHotsearch.addEventListener('click', function(e) { if (e.target !== refreshBtn) load(true); });
    })();
