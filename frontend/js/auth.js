/**
 * 示例校园墙 - 登录注册模块 (auth.js)
 * 处理登录和注册表单的提交、验证及跳转
 * 后端API返回格式：{ code: 200, message: '...', data: {...} }
 */

window.registerEmailVerificationEnabled = false;
var registerEmailCountdownTimer = null;

async function loadRegisterOptions() {
  var group = document.getElementById('emailVerifyGroup');
  if (!group) return;
  try {
    var res = await fetch('/api/auth/register-options');
    var data = await res.json();
    window.registerEmailVerificationEnabled = data.code === 200 && data.data && data.data.emailVerificationEnabled === true;
    group.style.display = window.registerEmailVerificationEnabled ? '' : 'none';
    var codeInput = document.getElementById('emailCode');
    if (codeInput) codeInput.required = window.registerEmailVerificationEnabled;
  } catch (err) {
    window.registerEmailVerificationEnabled = false;
...
window.sendRegisterEmailCode = async function() {
  if (!window.registerEmailVerificationEnabled) return;
  var emailEl = document.getElementById('email');
  var nicknameEl = document.getElementById('nickname');
  var email = emailEl ? emailEl.value.trim() : '';
  var btn = document.getElementById('sendEmailCodeBtn');
  var status = document.getElementById('emailCodeStatus');
  if (!email || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
    showToast('请先填写正确的邮箱地址', 'error');
  ...
  btn.textContent = '发送中...';
  if (status) {
    status.className = 'email-code-status';
    status.textContent = '正在发送验证码，请稍候...';
  }
  try {
    var res = await fetch('/api/auth/send-register-email-code', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ email: email, nickname: nicknameEl ? nicknameEl.value.trim() : '' })
    });
    var data = await res.json();
    if (data.code !== 200) {
    ...
    if (status) {
      status.className = 'email-code-status success';
      status.textContent = '验证码已发送，请检查收件箱（10分钟内有效）';
    }
    var remaining = 60;
    btn.textContent = remaining + ' 秒后重发';
    clearInterval(registerEmailCountdownTimer);
    registerEmailCountdownTimer = setInterval(function() {
      remaining--;
      if (remaining <= 0) {
        clearInterval(registerEmailCountdownTimer);
        btn.disabled = false;
    ...
    if (status) {
      status.className = 'email-code-status error';
      status.textContent = err.message || '验证码发送失败，请稍后重试';
    }
    showToast(err.message || '验证码发送失败，请稍后重试', 'error');
    btn.disabled = false;
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (registerForm) {
    loadRegisterOptions();
    var registerEmailEl = document.getElementById('email');
    if (registerEmailEl) {
      registerEmailEl.addEventListener('input', function() {
        var codeInput = document.getElementById('emailCode');
        var status = document.getElementById('emailCodeStatus');
        if (codeInput) codeInput.value = '';
        if (status && window.registerEmailVerificationEnabled) {
          status.className = 'email-code-status';
    ...
    registerForm.addEventListener('submit', function(e) {
      ...
      var emailCode = emailCodeEl ? emailCodeEl.value.trim() : '';
      if (window.registerEmailVerificationEnabled && !emailCode) {
        showToast('请输入邮箱验证码', 'error');
        if (emailCodeEl) emailCodeEl.focus();
        return;
      }
      if (!password) {
        showToast('请输入密码', 'error');
